...
void getFileNames (string dirName, vector<string>& imageFnames, string imgExt)
{
    ...
    if ((dir = opendir (dirName.c_str ())) != NULL) {
        while ((ent = readdir (dir)) != NULL) {
            // Избегаем фиктивных имен, которые считываются по умолчанию
...
int main ()
{
    ...
    HOGDescriptor hogDefault (Size (64, 128), // winSize
      Size (16, 16),                          // blocksize
      Size (8, 8),                            // blockStride,
      Size (8, 8),                            // cellSize,
      9,                                      // nbins,
    ...
    for (int i = 0; i < imagePaths.size (); i++) {
        ...
        hog.detectMultiScale (im, bboxes, weights, hitThreshold, winStride, padding, scale, finalThreshold, useMeanshiftGrouping);

        hogDefault.detectMultiScale (im, bboxes2, weights2, 0, winStride, padding, scale, finalThreshold, useMeanshiftGrouping);
